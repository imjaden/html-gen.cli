...
class TestRenderSummary(unittest.TestCase):
    ...
    def setUp(self):
        # 每用例独立临时目录 (pytest-xdist 多 worker 隔离)
        self.tmp = Path(tempfile.mkdtemp(prefix='_tmp_render_summary_'))
    ...
    def test_02_doc_summary(self):
        """doc: h2 章节数 + h3 子节数."""
        md = self.tmp / 't_doc.md'
        md.write_text('# 标题\n\n## 第一节\n内容\n\n### 子节\n内容\n\n## 第二节\n内容\n', encoding='utf-8')
        out = self.tmp / 't_doc.html'
    ...
    def test_03_slide_summary(self):
        """slide: h2 分页数 + 封面页."""
        md = self.tmp / 't_slide.md'
        md.write_text('# 封面\n\n## 页一\n内容\n\n## 页二\n内容\n', encoding='utf-8')
        out = self.tmp / 't_slide.html'
