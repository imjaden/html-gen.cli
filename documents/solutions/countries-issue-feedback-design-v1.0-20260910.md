# A 型表格 GitHub Issue 反馈通道设计 v1.0 (2026-09-10)
...
- **case 级配置放在数据 JSON 的 `options.feedback`**：`{"dataset","key","altKey"}` 随数据文件走
  （复用即随案例自描述），repo 由 CLI/env 注入合并 → 无需新增模板占位符（见 §4.2）。
...
- `openSplitAt(idx)` → `openSplitAt(idx, colKey)`；调用点 3 处：L552（pills 格）、L588 / L594
  （`col.onCellClick === 'split'` 的普通格）；
...
3. 解析 body：按 `### <label>` 切段 → 以 config `parse_fields` 映射回 id（精确匹配，见 §3.1）；
...
8. 回评：`gh issue comment <n> --body "✅ 已更新：<field> 旧→新（数据已重建，<hash 由维护者提交>）"`；
   `--close` 追加 `gh issue close <n>`。
...
| 用例 | 断言 |
|:---|:---|
| TC-01 默认不渲染 | `html-gen table` 不传 `--feedback-repo` → 分栏 header 无 `.sp-feedback`；HTML 无 `issues/new` |
