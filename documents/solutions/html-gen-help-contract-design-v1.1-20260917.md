# html-gen help 契约治理设计 v1.1（help 单一事实源）
...
> 基线链: v1.0（`4649053`，评审 CONDITIONAL PASS 80/B）→ **v1.1（折入 HG-SEC-161..167）**
> 评审报告: `documents/review/html-gen-help-contract-design-review-v1.0-20260917.md`
> 开放决策项: 无
...
- **table（P0）**：
  ...
  - 列类型补 `videos` 与 **`datetime`**（日期列专用排序，模板 L471/475/484 消费，features.md:159 已载）；
...
| table 列类型取值 | `layout-table.html` | `(?:col\|c\|col2)\.type\s*===\s*'([a-z]+)'` 与 `type\s*===\s*'([a-z]+)'` | CL012 |
...
| doc CLI | `html-gen.py` | `d\.add_argument\('--([a-z-]+)'` | CL013 |
| doc URL | `layout-doc.html` | `params\.get\('([a-z]+)'\)` | CL013 |
| slide 行为项 | `layout-slide.html` | 正则不可提取 → **显式清单 + 模板特征串存在性断言**（如 `.slide-toc-search`） | CL013 |
| slide CLI | `html-gen.py` | `s\.add_argument\('--([a-z-]+)'` | CL013 |
| knowledge CLI / item / groups | `html-gen.py` + `layout-knowledge.html` | `k\.add_argument\('--([a-z-]+)'`；`i?tem\.([a-zA-Z_]\w*)`；`\bg\.([a-zA-Z_]\w*)` | CL013 |
